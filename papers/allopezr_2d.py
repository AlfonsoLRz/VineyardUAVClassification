...
def get_allopezr_2d_model(img_size, num_classes, start_size=32, intermediate_activation='relu', kernel_size=3, strides=2):
    in_chunk = Input(shape=img_size)
    ...
    merge = Concatenate()([in_chunk, x])

    x = Conv2D(start_size * 1, kernel_size, strides=strides, padding="same")(merge)
    ...
    x = Flatten()(x)
    x = Dropout(0.1)(x)
    outputs = Dense(num_classes, activation="softmax", kernel_initializer=get_random_initializer())(x)
    model = Model([in_chunk, in_pixel], outputs)
